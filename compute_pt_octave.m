...
% Octave-compatible version: uses EXTRA struct instead of 4th return value
[pks,locs,extra]=findpeaks(begin_video);

% Extract prominence from EXTRA structure if available, otherwise use height
if isfield(extra, 'prominence')
    p = extra.prominence;
elseif isfield(extra, 'height')
    p = extra.height;
else
    % Fallback: calculate prominence manually as peak value - baseline
    p = pks;
end
